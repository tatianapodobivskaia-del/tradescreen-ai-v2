...
function generateDocumentScanPdfBlob(data: VisionScanResult): Blob {
  ...
  doc.text(`Scan time: ${formatScanDateTime(data.ts)}`, margin, y);
  y += 16;
  doc.setFontSize(8);
  ...
  const summaryBody: string[][] = [
    ["Document risk", data.document_risk.toUpperCase()],
    ["Document type", data.document?.document_type?.trim() || "—"],
    ["Entities found", String(data.entities_found)],
    ["Blocked", String(data.summary.blocked)],
    ["Flagged", String(data.summary.flagged)],
    ["Cleared", String(data.summary.cleared)],
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    const riskX = margin + doc.getTextWidth("Risk: ");
    setPdfRiskTextColor(doc, row.risk);
    doc.setFont("helvetica", "bold");
    doc.text((row.risk || "—").toUpperCase(), riskX, y);
    doc.setTextColor(0, 0, 0);
    doc.setFont("helvetica", "normal");
    const actionLabel = `   Action: `;
    const actionX = riskX + doc.getTextWidth((row.risk || "—").toUpperCase()) + 12;
    doc.text(`${actionLabel}${(row.action || "—").toUpperCase()}`, actionX, y);
    y += 14;
    ...
    y += 10;
    const reasonLines = doc.splitTextToSize((row.reasoning || "—").trim(), maxW);
    for (const wline of reasonLines) {
      ensureSpace(10);
      doc.text(wline, margin, y);
      y += 10;
    }

    if (row.cyrillic_variants?.length) {
      y += 4;
      doc.setFont("helvetica", "bold");
      doc.setFontSize(8);
      doc.text("Cyrillic variants:", margin, y);
      y += 10;
      doc.setFont("helvetica", "normal");
      const varText = row.cyrillic_variants.join("; ");
      for (const wline of doc.splitTextToSize(varText, maxW)) {
        ensureSpace(10);
        doc.text(wline, margin, y);
        y += 10;
      }
